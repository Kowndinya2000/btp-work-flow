...
function RenderObjectList() {
  ...
  html_str += '<p style="font-size:10px;line-height:100%"><a id="show_model_button" href="javascript:ShowModelAnnotations();">Auto annotate using ML Model</a></p>';

  html_str += '<p style="font-size:10px;line-height:100%"><a id="hide_model_button" href="javascript:DeleteAnnotationsByModel();">Delete annotations by ML Model</a></p>';
 ...
 *
 */
function ShowModelAnnotations(){
  if(model_anno==0){
  console.log(LM_xml);
  
  var collection_name = main_media.file_info.collection.toLowerCase()
  var url = '/annotate/auto/model?mode=' + main_media.file_info.mode + '&username=' + username + '&collection=' + collection_name + '&folder=' + main_media.file_info.dir_name + '&image=' + main_media.file_info.im_name;
  var json_req = $.getJSON(url, function(result, status, xhr){
 ...
 *
 */
function DeleteAnnotationsByModel(){
    ...
    for(var pp = 0; pp < num_obj; pp++) {
      ...
      var status = curr_obj.children("deleted").text();
      if(status!='1'){
        if(curr_obj.children("polygon").children("username").text() == "model") {
          xp = curr_obj.children("id").text();
          delIdx.push(parseInt(xp));
          xml_idx.push(pp);
        }
